...
public class NumeroUtil {
	...
	public static String porcentajeFormateado(double numero) {
		...
		int MAX_DECIMAL = 2;
		numFormateado += (cantDec == 0 ? "." : "")
				+ repeatString("0", MAX_DECIMAL - cantDec) + " %";
		return numFormateado;
	}

	public static String agregarCerosIzquierda(String numero, int nroDig) {
		String numFormat = "";
	...
	public static String numeroTarjetaFormateado(String nroTarjeta) {
		return nroTarjeta.substring(0, 4) + "-****-****-"
				+ nroTarjeta.substring(12);
	}

	public static String importeFormateado(String simbolo, String importe) {
		return simbolo
				+ (importe == null || importe.isEmpty() ? "0.00" : importe);
	}

	public static Integer menorValor(Integer[] arreglo) {
		Integer menorValor = arreglo[0];
		for (int i = 1; i < arreglo.length; i++) {
			if (arreglo[i] != null
					&& (menorValor == null || menorValor > arreglo[i]))
				menorValor = arreglo[i];
		}
		return menorValor;
	}

	public static boolean esNumero(char cadena) {
		try {
			Long.parseLong(String.valueOf(cadena));
			return true;
		} catch (NumberFormatException nfe) {
			return false;
		}
	...
	public static String separarLetrasDeNumeros(String cadena) {
		StringBuilder sb = new StringBuilder(cadena);

		for (int i = cadena.length() - 1; i > 0; i--) {
			if ((esNumero(cadena.charAt(i)) && esLetra(cadena.charAt(i - 1)))
					|| (esLetra(cadena.charAt(i)) && esNumero(cadena
							.charAt(i - 1))))
				sb.insert(i, " ");
		}
